> **⚠️ GENERATING MODE — read before choosing colors or fonts:**

> 1. Pick **one palette** from the 18 options in the table below and map all 5 theme keys to it. Write down the palette name before proceeding.

> 2. **Zero tolerance for out-of-palette colors** — every hex value in every slide file must be `theme.primary / secondary / accent / light / bg`. No exceptions.

> 3. Choose a **font pairing** from the Recommended Font Pairings table — do not default to Arial for everything.

> 4. Choose a **style recipe** (Sharp / Soft / Rounded / Pill) and apply it consistently across all slides.
...
### No Emoji — Use Native Shapes for Icons

**Do NOT use emoji or AI-generated images for small icons.** Both produce poor results at icon sizes: emoji render as blank squares on many systems; AI-gen images have inconsistent styles, excessive whitespace, and don't scale cleanly.

**Preferred approach — PptxGenJS native shapes:**

- **Icon containers**: colored OVAL with a white text symbol inside (`★` `▸` `●` `+` `1` `2` etc.)
- **Bullet markers**: small OVAL/RECTANGLE in accent color
- **Number circles**: OVAL background + centered digit text
- **Check/cross marks**: text characters `✓` `✗` in theme colors
- **Step indicators**: ROUNDED_RECTANGLE pill shapes
- **Accent bars**: thin RECTANGLE strips

```javascript
// Icon container: colored circle with symbol inside
slide.addShape(pres.shapes.OVAL, { x: 0.5, y: 1, w: 0.4, h: 0.4, fill: { color: theme.accent } });
slide.addText("★", { x: 0.5, y: 1, w: 0.4, h: 0.4, fontSize: 16, color: "FFFFFF", align: "center", valign: "middle" });

// Bullet marker
slide.addShape(pres.shapes.OVAL, { x: 0.5, y: 1.55, w: 0.15, h: 0.15, fill: { color: theme.secondary } });
slide.addText("Key point text", { x: 0.75, y: 1.5, w: 5, h: 0.3, fontSize: 13, fontFace: "Calibri" });

// Check mark in list
slide.addText("✓ Feature description", { x: 0.5, y: 2, w: 5, h: 0.3, fontSize: 12, color: theme.accent });
```

**image-gen reserved for large-format visuals only:** cover backgrounds, half-page illustrations, hero images. Never for icons, badges, or decorations under 1 inch.
