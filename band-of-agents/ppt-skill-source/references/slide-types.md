> **⚠️ GENERATING MODE — read before planning your slide outline:**
> 1. Your deck **must include at least one Section Divider** for each major chapter — this type is mandatory, not optional.
> 2. Every content slide must belong to a **different subtype** from the previous content slide (Text / Mixed Media / Data Visualization / Comparison / Timeline / Image Showcase).
> 3. TOC with 4+ sections → use **Card-Based or Sidebar** layout, not a plain vertical list.
> 4. Cover title must be **≥ 72pt**. Section divider number must be **≥ 72pt**. Never let adjacent text elements be within 20% of each other's size.
> 5. Every content slide **must have at least one non-text visual element** — icon, chart, image, or meaningful shape composition.
